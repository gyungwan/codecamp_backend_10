...
export class PaymentsService {
  ...
  async findAll() {
    ...
    await queryRunner.startTransaction('SERIALIZABLE'); //아이솔레이션 레벨 격리수준
    try {
      // 조회시 락을 걸고 조회함으로써, 다른 쿼리에서 조회 못하게 막음(대기시킴) => Select ~ For Update
      const payment = await queryRunner.manager.find(Payment, {
        lock: { mode: 'pessimistic_write' }, // pessimistic_write: 베타락(잠그기),   write_or_fail: 잠겼으면 관두기, partial_write: 잠긴것 패스하고 나머지 수행, for~~: postgres 전용
